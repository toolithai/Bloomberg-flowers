...
const categoryMap: Record<string, { label: string; photos: PhotoItem[] }> = {
  custom: { label: 'Custom Arrangements', photos: bouquets },
  centerpieces: { label: 'Centerpieces', photos: centerpieces },
  byo: { label: 'Celebratory Bouquets', photos: bouquetsWithPeople },
  workshops: { label: 'Workshops', photos: flowerParties },
  events: { label: 'Events', photos: events },
}
...
export default function GalleryCategory() {
  ...
  const breadcrumbJsonLd = {
    ...
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://www.bloombergflowers.com' },
      { '@type': 'ListItem', position: 2, name: 'Gallery', item: 'https://www.bloombergflowers.com/gallery' },
      { '@type': 'ListItem', position: 3, name: cat?.label ?? slug, item: `https://www.bloombergflowers.com/gallery/${slug}` },
    ],
  }
  ...
  return (
    <div className="gallery-page">
      ...
      <div className="gallery-header-bar">
        <h1>{cat.label}</h1>
        <span className="gallery-count">{cat.photos.length} photos</span>
      </div>
      ...
      {lightbox && (
        <div className="lightbox" onClick={() => setLightbox(null)}>
          <div className="lightbox-inner" onClick={e => e.stopPropagation()}>
            <Image src={lightbox.src} alt={lightbox.filename} fill style={{ objectFit: 'contain' }} quality={90} priority />
